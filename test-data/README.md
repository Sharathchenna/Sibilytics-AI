...
curl -X POST http://localhost:8000/api/upload-with-progress \
  -F "file=@test-data/test-signal-with-headers.csv"
```
...
See `docs/legacy/HEADER_DETECTION_TEST_RESULTS.md` for detailed test results and analysis.

## Adding New Test Cases

To add a new test case:

1. Create the test file in `test-data/`
2. Add it to the `tests` list in `backend/test_header_detection.py`:

```python
tests = [
    # ... existing tests ...
    (test_data_dir / "your-new-test-file.csv", True, "Description of your test"),
]
```
...
- `docs/legacy/SIGNAL_FILE_HEADER_DETECTION_ISSUE.md` - Original problem analysis
- `docs/legacy/SIGNAL_HEADER_FIX_SUMMARY.md` - Fix summary
- `docs/legacy/HEADER_FIX_CODE_CHANGES.md` - Code changes reference
- `docs/legacy/HEADER_DETECTION_TEST_RESULTS.md` - Test results and analysis
