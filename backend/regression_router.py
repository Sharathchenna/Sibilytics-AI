...
def parse_regression_file(contents: bytes, filename: str) -> pd.DataFrame:
    ...
    try:
        ...
        # Keep column indexing consistent with /api/upload response
        # so frontend-selected column ids map correctly in regression endpoints.
    ...
    try:
        ...
        for f_idx, feature in enumerate(x_cols):
            for i, target in enumerate(y_cols):
                ...
                ax.scatter(
                    X_test_np[sorted_idx, f_idx],
                    y_test_np[sorted_idx, i],
                    label="Actual",
                    alpha=0.6,
                )
                ax.scatter(
                    X_test_np[sorted_idx, f_idx],
                    y_pred_np[sorted_idx, i],
                    label="Predicted",
                    alpha=0.6,
                )
                ...
                plots.append(
                    {"feature": feature, "target": target, "plot": plot_to_base64(fig)}
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Linear regression error: {str(e)}"
        )
...
async def logistic_regression_analysis(
    ...
    try:
        ...
        for i, col in enumerate(X.columns):
            ...
            prob_plots.append({"feature": col, "plot": plot_to_base64(fig)})
        ...
        raise HTTPException(
            status_code=500, detail=f"Logistic regression error: {str(e)}"
        )
...
async def polynomial_regression_analysis(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Polynomial regression error: {str(e)}"
        )
